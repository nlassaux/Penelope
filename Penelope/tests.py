...
class LoggedAsTeacherCase(TestCase):
    def setUp(self):
            ...
            # Create a student (an object to be manipulated)
            self.user = User.objects.create_user(username=studentusername, password=password)

            # Create a teacher (an object to be manipulated)
            self.user = User.objects.create_user(username=teacherusername, password=password)
            userprofile = UserProfile.objects.get(user__username=teacherusername)
            userprofile.status = 'teacher'
            userprofile.save()
    ...
    def test_changeowner(self):
        ...
        # Test if post and redirection after are ok. We set owner as teacherusername
        response = self.client.post('/1/changeowner/', {'owner': User.objects.get(username='teacherusername').id})
        self.assertRedirects(response, '/')

        # Test if result is ok
        course = Course.objects.get(id='1')
        self.assertEqual(course.owner.id, User.objects.get(username='teacherusername').id)
